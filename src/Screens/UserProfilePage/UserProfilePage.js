...
const UserProfilePage = () => {
  ...
  const onUploadProfilePicture = (event) => {
    ...
    GetPreSignProfilePictureURL(fileExtension)
      .then(({ data }) => {
        UploadProfilePicture(data.url, file)
          .then(() => {
            const url = data.url.split('?')[0];
            const urlPaths = url.split('/');
            const uploadID = urlPaths.slice(urlPaths.length - 2);
            SyncProfilePicture(uploadID.join('/'))
              .then(() => {
  ...
  const onSaveProfile = () => {
    ...
    editUserProfileDetails(FieldsToUpdate)
      .then(() => {
        ...
        <If condition={isEditProfile}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <ProfileEditContainer
              userDetails={profileDetailsInEdit}
              onProfileEdit={(key, value) => {
                if (key === 'name') {
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            >
              <LinkStyle style={{ color: '#FA163F' }} onClick={onEditProfile}>
                Edit Profile
              </LinkStyle>
